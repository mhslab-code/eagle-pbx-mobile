...
- `docker-compose.yml`: serviços de conteúdo estático e contador;
- `counter/server.py`: download autenticado, lista positiva de APKs e
  contabilização persistente sem registrar credenciais.

O estado versionado corresponde ao catálogo publicado até a versão `0.1.63`.
O banco do contador em `counter-data/`, o arquivo `htpasswd` e os próprios APKs
